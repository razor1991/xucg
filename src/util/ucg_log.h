 ...
 *                          log level order : FATAL < ERROR < WARN < INFO < DEBUG < TARCE
 * @param [in] prefix       the prefix in log output
 */
void ucg_log_configure(ucg_log_level_t max_level, const char *prefix);

/**
 * @brief Get the global log component, just use for marco as follow
 */
...
#define ucg_fatal(_fmt, ...)    ucg_log_detail(UCG_LOG_LEVEL_FATAL, _fmt, ##__VA_ARGS__)
#define ucg_error(_fmt, ...)    ucg_log_detail(UCG_LOG_LEVEL_ERROR, _fmt, ##__VA_ARGS__)
#define ucg_warn(_fmt, ...)     ucg_log_detail(UCG_LOG_LEVEL_WARN,  _fmt, ##__VA_ARGS__)
#define ucg_info(_fmt, ...)     ucg_log_detail(UCG_LOG_LEVEL_INFO,  _fmt, ##__VA_ARGS__)
#define ucg_debug(_fmt, ...)    ucg_log_detail(UCG_LOG_LEVEL_DEBUG, _fmt, ##__VA_ARGS__)
#define ucg_trace(_fmt, ...)    ucg_log_detail(UCG_LOG_LEVEL_TRACE, _fmt, ##__VA_ARGS__)
