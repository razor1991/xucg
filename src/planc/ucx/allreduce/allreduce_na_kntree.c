...
static ucg_status_t ucg_planc_ucx_allreduce_na_kntree_check(ucg_vgroup_t *vgroup,
                                                            const ucg_coll_args_t *args)
{
    ucg_op_flag_t flags = args->allreduce.op->flags;
    if (!(flags & UCG_OP_FLAG_IS_COMMUTATIVE)) {
        ucg_info("Allreduce na_kntree don't support non-commutative op");
        return UCG_ERR_UNSUPPORTED;
    }
    return UCG_OK;
}

ucg_plan_meta_op_t* ucg_planc_ucx_allreduce_na_kntree_op_new(ucg_planc_ucx_group_t* ucx_group,
                                                             ucg_vgroup_t *vgroup,
                                                             const ucg_coll_args_t* args,
                                                       ...
                                                       ucg_plan_op_t **op)
{
    ...
    ucg_status_t status;
    status = ucg_planc_ucx_allreduce_na_kntree_check(vgroup, args);
    if (status != UCG_OK) {
        return UCG_ERR_UNSUPPORTED;
    }
