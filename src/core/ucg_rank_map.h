 ...
 * stride-type can be optimized to full-type which stride is 1, which reduces the
 * compute overhead.
 *
 * @param [inout] map       Rank map.
 * @param [in]    array     Ingored if not array-type. Otherwise when array-type
...
/**
 * @brief Map src-rank to dst-rank
 *
